	...
	$.fn.extend({
		...
		extendedSVG: function(options) {
			...
			var SVG = (function(){
				...
				{				
					if(o.isPlotDataPoints || o.isPlotHeatMap)
					{
						this.loadDataPoints();
				...
				{
					if(this.svgTrajectory.children().length == 0)
					{			
						var paths = [
							{x: 245, y: 175}, 	{x: 235, y: 175}, 	{x: 235, y: 200}, 	{x: 420, 	y: 200},
							{x: 420, y: 175}, 	{x: 455, y: 175}, 	{x: 525, y: 90}, 	{x: 512, 	y: 90}, 
							{x: 460, y: 150}, 	{x: 160, y: 150},	{x: 160, y: 315},	{x: 180, 	y: 315},
						...
						var d = 'M' + paths[0].x + ' ' + paths[0].y + ' ';
						
						for(var i = 1; i < paths.length; i++)
						{
							d = d + 'L' + paths[i].x + ' ' + paths[i].y + ' ';						
						}
						...
						this.svgTrajectory.append(path);
						this.animateTrajectory(true);
					}
				};
				
				SVG.prototype.animateTrajectory = function(isPlay)
				{					
					var path = this.svgTrajectory.find('path').first();

					if(isPlay)
					{
						path.attr('class', 'play-animation');
					}
					else
					{
						path.attr('class', 'pause-animation');
					}
				};
				...
				{					
					...
					this.divWrapper.find('div.floor-plan-buttons button').bind('mouseup', function(event)
					{
						...
						else if(button.hasClass('playTrajectory'))
						{
							self.plotTrajectory();
							self.animateTrajectory(true);
							
							button.removeClass('playTrajectory');
							button.addClass('stopTrajectory');
							
							var icon = button.children('i');
								icon.removeClass('fa-play');
								icon.addClass('fa-stop');
						}
						else if(button.hasClass('stopTrajectory'))
						{
							self.animateTrajectory(false);
							self.svgTrajectory.empty();
							
							button.removeClass('stopTrajectory');
							button.addClass('playTrajectory');
					...
					$(self.divWrapper).bind('mouseup', function(event)
					{							
						var screenPoint = self.svgMain[0].createSVGPoint();
							screenPoint.x = event.clientX;
							screenPoint.y = event.clientY;
							
						var svgPoint = screenToSvg(self.svgMain[0], screenPoint);
				...
				SVG.prototype.createNodes = function(element)
				{
					...
					var playTrajectoryButton = $('<button>');
						playTrajectoryButton.attr('role', 'button');
						playTrajectoryButton.addClass('playTrajectory');
						playTrajectoryButton.addClass('btn');
						playTrajectoryButton.addClass('btn-icon');
						playTrajectoryButton.addClass('btn-success');
						playTrajectoryButton.addClass('waves-effect');
						playTrajectoryButton.addClass('waves-light');
						playTrajectoryButton.append(
							$('<i>').addClass('fa')
									.addClass('fa-play')
						);
					...
					buttons.append(playTrajectoryButton);
					...
					this.svgMain.attr('class', 'svgMain');
					...
					this.svgPlot.attr('class', 'plotLayer');
					...
					this.svgHeatMap.attr('class', 'heatMapLayer');
					...
					this.svgInteract.attr('class', 'interactiveLayer');
					...
					this.svgTrajectory.attr('class', 'trajectoryLayer');
